...
_PRIVATE_STORAGE_ROOT_ENV = os.getenv("PRIVATE_STORAGE_ROOT", "").strip()
if _PRIVATE_STORAGE_ROOT_ENV:
    app.config["PRIVATE_STORAGE_ROOT"] = (
        _PRIVATE_STORAGE_ROOT_ENV
        if os.path.isabs(_PRIVATE_STORAGE_ROOT_ENV)
        else os.path.join(BASE_DIR, _PRIVATE_STORAGE_ROOT_ENV)
    )
else:
    app.config["PRIVATE_STORAGE_ROOT"] = os.path.join(BASE_DIR, "private")
app.config["PRIVATE_STORAGE_ROOT"] = os.path.abspath(app.config["PRIVATE_STORAGE_ROOT"])

# Las facturas ya no se guardan en static. Se almacenan en private/facturas
# y solo se sirven mediante /facturas/<id>/ver después de validar permisos.
app.config["INVOICE_FOLDER"] = os.path.join(
    app.config["PRIVATE_STORAGE_ROOT"],
    "facturas"
)

# Carpeta antigua. Se conserva solo para compatibilidad y migración de datos
# previos; no debe usarse para facturas nuevas.
app.config["LEGACY_INVOICE_STATIC_FOLDER"] = os.path.join(
    BASE_DIR,
    "static",
    "docs",
    "facturas"
...
os.makedirs(app.config["PRIVATE_STORAGE_ROOT"], exist_ok=True)
...
def tabla_existe_db(cursor, nombre_tabla):
    """Indica si una tabla existe en la base actual."""
...
def columna_existe_db(cursor, nombre_tabla, nombre_columna):
    """Indica si una columna existe, evitando errores en bases antiguas."""

    if not tabla_existe_db(cursor, nombre_tabla):
        return False

    cursor.execute(f"PRAGMA table_info({nombre_tabla})")
    return any(str(fila[1]) == nombre_columna for fila in cursor.fetchall())


def listar_recursos_static_bd(cursor):
    """Recursos locales reutilizables para no duplicar imágenes, manuales o modelos 3D."""
    ...
    def agregar_unico(grupo, ruta, etiqueta, extra=None):
        ruta_normalizada = normalizar_ruta_static_documento(ruta)
        if not ruta_normalizada:
            return
        vistos = {item["ruta"] for item in recursos[grupo]}
        if ruta_normalizada in vistos:
            return
        recursos[grupo].append({
            "ruta": ruta_normalizada,
            "etiqueta": str(etiqueta or ruta_normalizada).strip() or ruta_normalizada,
            "extra": str(extra or "").strip(),
        })

    if tabla_existe_db(cursor, "vehiculos") and columna_existe_db(cursor, "vehiculos", "imagen"):
        cursor.execute("""
            SELECT imagen, marca, modelo, anio, codigo_catalogo
            FROM vehiculos
            WHERE TRIM(COALESCE(imagen, '')) != ''
            ORDER BY datetime(COALESCE(actualizado_en, creado_en)) DESC, id DESC
        """)
        for fila in cursor.fetchall():
            agregar_unico(
                "imagenes_vehiculos",
                fila["imagen"],
                f"{fila['marca'] or ''} {fila['modelo'] or ''} {fila['anio'] or ''}".strip(),
                fila["codigo_catalogo"],
            )

    if tabla_existe_db(cursor, "vehiculo_modelos") and columna_existe_db(cursor, "vehiculo_modelos", "modelo_3d"):
        cursor.execute("""
            SELECT modelo_3d, marca, modelo, anio, modelo_3d_tipo
            FROM vehiculo_modelos
            WHERE TRIM(COALESCE(modelo_3d, '')) != ''
            ORDER BY datetime(COALESCE(actualizado_en, creado_en)) DESC, id DESC
        """)
        for fila in cursor.fetchall():
            agregar_unico(
                ...
                fila["modelo_3d"],
                f"{fila['marca'] or ''} {fila['modelo'] or ''} {fila['anio'] or ''}".strip(),
                fila["modelo_3d_tipo"] or "3D",
            )

    if tabla_existe_db(cursor, "manuales_modelo") and columna_existe_db(cursor, "manuales_modelo", "archivo"):
        cursor.execute("""
            SELECT manuales_modelo.archivo, manuales_modelo.titulo, vehiculo_modelos.marca, vehiculo_modelos.modelo, vehiculo_modelos.anio
            FROM manuales_modelo
            LEFT JOIN vehiculo_modelos ON vehiculo_modelos.id = manuales_modelo.modelo_id
            WHERE TRIM(COALESCE(manuales_modelo.archivo, '')) != ''
        ...
        for fila in cursor.fetchall():
            agregar_unico(
                "manuales",
                fila["archivo"],
                fila["titulo"] or "Manual",
                f"{fila['marca'] or ''} {fila['modelo'] or ''} {fila['anio'] or ''}".strip(),
            )

    if tabla_existe_db(cursor, "articulos") and columna_existe_db(cursor, "articulos", "imagen"):
        cursor.execute("""
            SELECT imagen, nombre, codigo_articulo, categoria
            FROM articulos
            WHERE TRIM(COALESCE(imagen, '')) != ''
            ORDER BY datetime(COALESCE(actualizado_en, creado_en)) DESC, id DESC
        """)
        for fila in cursor.fetchall():
            agregar_unico(
                "imagenes_articulos",
                fila["imagen"],
                fila["nombre"] or "Artículo",
                fila["codigo_articulo"] or fila["categoria"] or "",
            )

    return recursos


def _contar_referencia_columna(cursor, tabla, columna, ruta):
    if not tabla_existe_db(cursor, tabla) or not columna_existe_db(cursor, tabla, columna):
        return 0

    cursor.execute(
        f"SELECT COUNT(*) FROM {tabla} WHERE REPLACE(TRIM(COALESCE({columna}, '')), '\\', '/') IN (?, ?, ?)",
        (ruta, f"static/{ruta}", f"/static/{ruta}"),
    )
    return int(cursor.fetchone()[0] or 0)


def contar_referencias_recurso_static(cursor, ruta):
    """Cuenta referencias de una ruta local dentro de tablas conocidas."""

    ruta = normalizar_ruta_static_documento(ruta)
    if not ruta:
        return 0
    ...
    referencias = (
        ("vehiculos", "imagen"),
        ("vehiculos", "modelo_3d"),
        ("vehiculo_modelos", "modelo_3d"),
        ("manuales_modelo", "archivo"),
        ("manuales_vehiculo", "archivo"),
        ("facturas_vehiculo", "archivo"),
        ("articulos", "imagen"),
        ("establecimientos", "imagen"),
    )

    return sum(_contar_referencia_columna(cursor, tabla, columna, ruta) for tabla, columna in referencias)


def eliminar_archivo_static_si_no_referenciado(cursor, ruta):
    """Borra un archivo local solo si ya no está referenciado por ningún registro."""

    ruta = normalizar_ruta_static_documento(ruta)
    if not ruta or contar_referencias_recurso_static(cursor, ruta) > 0:
        return False

    ruta_absoluta = os.path.abspath(os.path.join(app.static_folder, ruta))
    static_root = os.path.abspath(app.static_folder)

    if not ruta_absoluta.startswith(static_root + os.sep):
        return False

    if os.path.basename(ruta_absoluta).lower() == ".gitkeep":
        return False
...
# ================= FACTURAS =================
# La generación y entrega segura de facturas vive en vinova/services/facturas.py
...
def normalizar_placa_vehiculo(valor):
    """Normaliza una placa/matrícula sin obligarla."""
...
DEFAULT_ESTABLECIMIENTO_LAT = -2.170998
DEFAULT_ESTABLECIMIENTO_LNG = -79.922359
...
def reparar_coordenadas_establecimientos(cursor):
    """Evita establecimientos activos sin lat/lng para que siempre puedan aparecer en el mapa."""
    ...
    try:
        cursor.execute("""
            ...
            WHERE lat IS NULL
               ...
               OR (lat = 0 AND lng = 0)
        """, (
            DEFAULT_ESTABLECIMIENTO_LAT,
            DEFAULT_ESTABLECIMIENTO_LNG,
            fecha_actual(),
        ))
...
def listar_establecimientos(cursor, incluir_inactivos=False, solo_tipo=None):
    ...
    try:
        reparar_coordenadas_establecimientos(cursor)
...
# La lógica vive en vinova/services/notificaciones.py
